...
# --- 1. Per-ancestry study sumstats (shared across ALL variants) ------
...
# --- 5. Global ancestry colour palette (shared by all variants) ------
ancestry_colours <- c(
  "EUR"  = "#EE2C2C",
  "AFR"  = "#1874CD",
  "NAT"  = "#008B45",
...
meta_label <- c(random = "Random Effects", fixed = "Fixed Effects")
...
build_meta <- function(vid, type) {
  meta_hits %>%
    ...
    transmute(study = "Meta-analysis", ancestry = "META",
...
make_forest <- function(gene, variant_id, label, meta_type) {
  ...
  title <- sprintf("%s %s %s Meta-analysis using TRACTOR sumstats per Ancestry",
  ...
  df_table <- df_all %>%
    mutate(
      ...
      # N already counts haplotypes (Tractor dosages), so no factor of 2.
      MAC = pmin(AF, 1 - AF) * n,
  ...
  fname <- sprintf("forest_tractor_%s_%s_%s_%s.png",
